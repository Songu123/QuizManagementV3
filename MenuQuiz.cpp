    ...
    try {
        ...
        if (sqr.num_rows() > 0) {
            ...
            cout << std::setw(5) << std::left << "STT" <<'|'
                 ...
                 << std::setw(50) << std::left << "TÊN ĐỀ" << '|' << std::endl;
            ...
            for (int i = 0; i < sqr.num_rows(); ++i) {
                ...
                cout << '|' << std::setw(5) << std::left << (i + 1) << '|'
                     ...
                     << std::setw(50) << std::left << nameQuiz << '|' << endl;
                drawHorizontalLine(70);
            }
            cout << "Chọn id đề: ";
            cin >> quiz_id;
...
//                Truy vấn thông tin vào bảng result
...
//                Truy vấn result_id từ bảng result trong Database
...
int MenuQuiz::sumPointQuiz(string time_User){
    int pointQuiz;
    try {
        string timeUser = (string) time_User;
        Database::connectToDatabase();
        ...
        if(!sqr_query_Quiz_User.empty()){
            ...
            for (int i = 0; i < sqr_query_Quiz_User.num_rows(); ++i) {
                ...
                cout << '|' << std::setw(5) << std::left << (i + 1) << '|'
                     ...
                     << std::setw(10) << std::left << quizID << '|'
                     << std::setw(50) << std::left << nameQuiz << '|'
                     << std::setw(3) << std::left << point << "/" << sumPointQuiz(timeQuiz) <<  '|' << std::endl;
                drawHorizontalLine(tableWidth);
            }
        }else {
